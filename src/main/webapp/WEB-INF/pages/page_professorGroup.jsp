                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">专家(审批)用户组管理</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">添加专家(审批)用户组</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <form id="groupOfUserForm">
                            <div class="form-group">
                                <label>项目主体</label>
                                <select class="form-control" name="projectEntity" id="projectEntity">
                                </select>
                            </div>
                            <div class="form-group">
                                <label>用户组</label>
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary" id="addNewProfessorGroup"
                                data-dismiss="modal">
                            添加专家(审批)用户组
                        </button>
                    </div>
        ...
        <div class="modal fade" id="modal-researchSecretaryGroupInf" style="display: none;" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content" id="app">
                    <div class="modal-header">
                        <h4 class="modal-title">更改专家(审批)用户组信息</h4>
                    ...
                    <div class="modal-body">
                        <!-- 内容开始 -->
                        <form id="groupOfResearchDepartment">
                            <div class="form-group">
                                <label>所属二级部门</label>
                                <input type="text" class="form-control" placeholder="Project Entity Name"
                                       id="projectEntityName" name="projectEntityName" disabled>
                            </div>
                            <div class="form-group">
                                <label>群组名称</label>
                                <select class="form-control" name="groupOfUserUuid" id="groupOfResearchDepartmentChange">
                                </select>
                            </div>
                        </form>
                        <!-- 内容结束 -->
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary" id="alterBpmnInf"
                                onclick="changeGroupOfResearchDepartment()" data-dismiss="modal">更改专家(审批)用户组
                        </button>
                    </div>
                </div>
                <!-- /.modal-content -->
            ...
            <div class="container-fluid">
                ...
                <div class="card card-primary card-outline">
                    <div class="card-header">
                        <h3 class="card-title" style="margin-top: 5px;">专家(审批)用户组列表</h3>
                        <!-- 右侧按钮 -->
                    ...
                    <div class="card-body">
                        ...
                        <div style="margin-bottom: 15px; display: none;" id="filterBar">
                            <div class="row">
                                <div class="col-2">
                                    <lable class="col-form-label">专家(审批)用户群组ID</lable>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#groupOfResearchDepartmentChange').select2({
            ajax: {
                url: '${pageContext.request.contextPath}/getGroupOfUserSelect',
                dataType: 'json'
                // Additional AJAX parameters go here; see the end of this chapter for the full code of this example
        ...
        $('#projectEntity').select2({
            ajax: {
                url: '${pageContext.request.contextPath}/getAllProjectEntitySelect',
                dataType: 'json'
                // Additional AJAX parameters go here; see the end of this chapter for the full code of this example
            }
        });
        ...
        $("#jsGrid1").jsGrid({
            ...
            controller: {
                loadData: function (filter) {
                    ...
                    return $.ajax({
                        type: "GET",
                        url: "${pageContext.request.contextPath}/getGroupOfProfessor",
                        data: filter,
                        dataType: "JSON"
                    })
            ...
            fields: [
                ...
                {
                    name: "projectEntity.projectEntityName",
                    type: "text",
                    width: 200,
                    title: "所属项目主体",
                ...
                {name: "group.type", type: "text", width: 100, title: "群组类型"},
                {
                    ...
                    itemTemplate: function (value, item) {
                        ...
                        var $editGroupOfSecretary = $("<button>").attr({class: "btn btn-default btn-flat"}).append($("<i></i>").attr({class: "far fa-edit"}))
                            .click(function (e) {
                                if(item.projectEntity === null){
                                    toEditGroupOfResearchDepartment('null', item.group.name, item.group.id, item.id);
                                } else {
                                    toEditGroupOfResearchDepartment(item.projectEntity.projectEntityName, item.group.name, item.group.id, item.id);
                                }
                                e.stopPropagation();
                            });
                        ...
                        var $deleteGroupOfSecretary = $("<button>").attr({class: "btn btn-default btn-flat"}).append($("<i></i>").attr({class: "far fa-trash-alt"}))
                            .click(function (e) {
                                toDeleteGroupOfResearchDepartment(item.id);
                                e.stopPropagation();
                            });
    ...
    $("#addNewProfessorGroup").click(function () {
        toastr.info('正在添加科研秘书用户组信息！')
        var formData = new FormData($("#groupOfUserForm")[0]);
        formData.append("type", "professor");
        $.ajax({
            url: "${pageContext.request.contextPath}/addGroupOfDepartment",
            type: "POST",
            data: formData,
            ...
            success: function (data) {
                ...
                if (data.code === 400) {
                    console.log(data);
                    toastr.error("添加专家用户组信息失败！")
                } else if (data.code === 200) {
                    console.log(data);
                    // $('#image-uploading').fadeOut("slow");
                    toastr.success("添加专家用户组信息成功！")
    ...
    function toEditGroupOfResearchDepartment(projectEntityName, groupOfResearchDepartmentName, groupOfResearchDepartmentUuid, groupOfResearchDepartmentId) {
        if(projectEntityName === 'null'){
            $("#projectEntityName").val("不区分项目主体");
        } else {
            $("#projectEntityName").val(projectEntityName);
        }
        var groupOfSecretaryChange = $('#groupOfResearchDepartmentChange');
        var option = new Option(groupOfResearchDepartmentName, groupOfResearchDepartmentUuid, true, true);
        ...
        groupOfSecretaryChange.trigger({
            ...
            params: {
                data: {
                    "id": groupOfResearchDepartmentUuid,
                    "text": groupOfResearchDepartmentName,
                }
            }
        });
        global_data.groupOfResearchDepartmentId = groupOfResearchDepartmentId;
    ...
    function toDeleteGroupOfResearchDepartment(groupOfResearchDepartmentId) {
        var json_data = {groupOfResearchDepartmentId: groupOfResearchDepartmentId}
        $.ajax({
            type: 'post',
            url: '${pageContext.request.contextPath }/deleteGroupOfResearchDepartmentInf',
            ...
            success: function (data) {
                ...
                if (data.code === 400) {
                    console.log(data);
                    toastr.error("专家用户群组删除失败!");
                } else if (data.code === 200) {
                    console.log(data);
                    // $('#image-uploading').fadeOut("slow");
                    toastr.success("专家用户群组删除成功！");
                    $("#jsGrid1").jsGrid("loadData");
                }
            },
            error: function (e) {
    ...
    function changeGroupOfResearchDepartment() {
        var formData = new FormData($("#groupOfResearchDepartment")[0]);
        formData.append("groupOfResearchDepartmentId", global_data.groupOfResearchDepartmentId);

        $.ajax({
            url: "${pageContext.request.contextPath}/changeGroupOfResearchDepartment",
            type: "POST",
            data: formData,
            processData: false, //告诉ajax不要处理和编码这些数据，直接提交
            contentType: false, //不使用默认的内容类型
